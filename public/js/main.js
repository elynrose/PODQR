...
$(function() {
    ...
    // Check if we have a QR code to load (skip demo objects)
    var hasQrCode = window.location.search.includes('qr_code=');
    
    // Add demo objects only if no QR code is being loaded
    if (!hasQrCode) {
    ...
    $('input[name=form_shirt_type]', '#leftLayoutContainer').on('change', function() {
        ...
        if (shirtData[this.value]['color'].length < 2) {
            ...
            $('#colorButtonGroup').html(data).hide();
        } else {
            $('#div_colors_title').show();
            $('#colorButtonGroup').html(data).show();
            
            // Add Bootstrap 5 compatible event handlers to the newly created color buttons
            if (typeof window.reattachColorButtonHandlers === 'function') {
    ...
    $('#colorButtonGroup').on('change', function(e) {
    ...
    function change_shirt_color() {
        ...
        console.log('change_shirt_color called with color:', selectedColor);
        
        // Use the new dynamic color switching system
        if (typeof window.switchToClothesTypeWithColor === 'function') {
            window.switchToClothesTypeWithColor(selectedColor);
        } else {
            console.log('switchToClothesTypeWithColor function not available');
        }
    }
    ...
    function show_cvtoolbox_warn(value) {
        var alertHtml = `
            <div class="alert alert-warning alert-dismissible fade show" role="alert" style="padding: 0.5rem 1rem; margin: 0;">
                <span>${value}</span>
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close" style="float: right;"></button>
            </div>
        `;
        $('.cvtoolbox_info div span', '#centerLayoutContainer').html(alertHtml);
